...
class Trainer(object):
    ...
    def run(self, is_training):
        ...
        for i, (train_tasks, val_tasks) in enumerate(
            ...
            (pre_train_measurements, adapted_params, embeddings, DSNLoss
                ) = self._meta_learner.adapt(train_tasks)
            post_val_measurements = self._meta_learner.step(
                adapted_params, embeddings, val_tasks, is_training, DSNLoss)
            ...
            if (i % self._log_interval == 0 or i == 1):
                ...
                _entropy_mean = np.mean(self._meta_learner._entropy_mean)
                self._meta_learner._entropy_mean = []
                _entropyH2_mean = np.mean(self._meta_learner._entropyH2_mean)
                self._meta_learner._entropyH2_mean = []
                _entropyH1_mean = np.mean(self._meta_learner._entropyH1_mean)
                self._meta_learner._entropyH1_mean = []
                self.log_output(
                    pre_val_measurements, pre_train_measurements,
                    post_val_measurements, post_train_measurements, 
                    i, _grads_mean,_entropy_mean,_entropyH2_mean,_entropyH1_mean)
    ...
    def write_tensorboard(self, pre_val_measurements, pre_train_measurements,
                          post_val_measurements, post_train_measurements, 
                          iteration, embedding_grads_mean=None,_entropy_mean = None,_entropyH2_mean = None,_entropyH1_mean = None):
        ...
        if _entropy_mean is not None:
            self._writer.add_scalar(
                '_entropy_mean', _entropy_mean, iteration)
        if _entropyH2_mean is not None:
            self._writer.add_scalar(
                '_entropyH2_mean', _entropyH2_mean, iteration)
        if _entropyH1_mean is not None:
            self._writer.add_scalar(
                '_entropyH1_mean', _entropyH1_mean, iteration)
    ...
    def log_output(self, pre_val_measurements, pre_train_measurements,
        ...
        if _entropy_mean is not None:
            log_str = (log_str + '_entropy_mean after: {:.3f} '
                                    ''.format(_entropy_mean))
        if _entropyH2_mean is not None:
            log_str = (log_str + '_entropyH2_mean after: {:.3f} '
                                    ''.format(_entropyH2_mean))
        print(log_str)
