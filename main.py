...
from maml.models.functions import SIMSE, DiffLoss, MSE
...
from maml.models.model_compat import DSN
...
def main(args):
    ...
    if args.embedding_type == '':
        ...
        raise ValueError('Unrecognized embedding type {}'.format(
            args.embedding_type))
    if args.dsnNum:
        DSN_net = DSN(inputsize=args.embedding_hidden_size, outputsize=args.embedding_hidden_size)


        DSN_net.to(args.device)
        loss_classification = torch.nn.CrossEntropyLoss()
        loss_recon1 = MSE().to(args.device)
        loss_recon2 = SIMSE().to(args.device)
        loss_diff = DiffLoss().to(args.device)
        loss_similarity = torch.nn.CrossEntropyLoss()

        loss_list = [loss_classification, loss_recon1, loss_recon2, loss_diff, loss_similarity]
        optimizers = (torch.optim.Adam(model_parameters, lr=args.slow_lr),
                      torch.optim.Adam(embedding_parameters, lr=args.slow_lr),
                      torch.optim.Adam(DSN_net.parameters(), lr=args.slow_lr))
    else:
        DSN_net = None
        loss_list = []
        # optimizers = None
    ...
    meta_learner = MetaLearner(
        ...
        DAnet=DSN_net, Loss_list=loss_list, dsn_num=args.dsnNum, dsnNetNum=args.dsnLayerNum)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--miml-model', type=str2bool, default=False,  # default=False,
    ...
    # Optimization
    parser.add_argument('--num-batches', type=int, default=600000,
    ...
    parser.add_argument('--num-samples-per-class', type=int, default=5,  # 1,
    ...
    # entropy loss
    parser.add_argument('--entropy-alpha', type=float, default=0.0,
                        help='')
    # DSN net
    parser.add_argument('--dsnNum', type=float, default=0.0,
                        help='')
    parser.add_argument('--dsnLayerNum', type=int, default=4,
                        help='')
    ...
    assert not (args.miml_model and args.maml_model)

    # miml model: gated conv + convGRU
    if args.miml_model is True:
        print('Use MIML-DA')
